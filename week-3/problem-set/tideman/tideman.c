  ...
  for (int i = 0; i < candidate_count; i++) {
    for (int j = 0; j < candidate_count; j++) {
      if (preferences[i][j] > preferences[j][i]) {
        pair_count++;
        pairs[pair_count - 1] = (pair){.winner = i, .loser = j};
      }
    }
  }
  return;
...
void sort_pairs(void) {
  for (int i = 0; i < pair_count - 1; i++) {
    for (int j = i + 1; j < pair_count; j++) {
      pair temp = pairs[i];
      int gap_i = preferences[pairs[i].winner][pairs[i].loser] -
                  preferences[pairs[i].loser][pairs[i].winner];
      int gap_j = preferences[pairs[j].winner][pairs[j].loser] -
...
bool trace_leads_to_winner(int origin_winner, int loser) {
  ...
  for (int i = 0; i < candidate_count; i++) {
    if (locked[loser][i]) {
      if (trace_leads_to_winner(origin_winner, i)) {
        return true;
      }
    }
  }
